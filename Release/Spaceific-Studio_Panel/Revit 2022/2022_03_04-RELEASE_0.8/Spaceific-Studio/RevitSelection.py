...
def getValueByParameterName(el, inName, doc, *args, **kwargs):
	...
	forgeTypeId = DB.ForgeTypeId(DB.UnitTypeId.Millimeters.TypeId)
	...
	if bip:
		...
		if parameterVP.IsDoubleValueSupported(el):
			returnValue = DB.UnitUtils.ConvertFromInternalUnits(parameterVP.GetDoubleValue(el), forgeTypeId)
		...
		if not typeElement:
			...
			if parameter:					
				if parameter.StorageType == DB.StorageType.Double:
					returnValueAsString = "{0}, {4}, {1}, {2}, {3:.4f}".format(el.Name if hasattr(el, "Name") else el.FamilyName, el.Id, parameter.Definition.Name, DB.UnitUtils.ConvertFromInternalUnits(parameter.AsDouble(), forgeTypeId), el.Id)
					returnValue = DB.UnitUtils.ConvertFromInternalUnits(parameter.AsDouble(), forgeTypeId)
...
def getValuesByParameterName(inElements, inName, doc, *args, **kwargs):
	...
	for el in inElements:
		...
		if bip:
			...
			parameterVP = DB.ParameterValueProvider(param_ID)
			if parameterVP.IsDoubleValueSupported(el):
				returnValues.append(DB.UnitUtils.ConvertFromInternalUnits(parameterVP.GetDoubleValue(el), forgeTypeId))
			elif parameterVP.IsIntegerValueSupported(el):
			...
			if not typeElement:
				...
				if parameter:					
					if parameter.StorageType == DB.StorageType.Double:
						returnValuesAsString.append("{0}, {4}, {1}, {2}, {3:.4f}".format(el.Name if hasattr(el, "Name") else el.FamilyName, el.Id, parameter.Definition.Name, DB.UnitUtils.ConvertFromInternalUnits(parameter.AsDouble(), forgeTypeId), el.Id))
						returnValues.append(DB.UnitUtils.ConvertFromInternalUnits(parameter.AsDouble(), forgeTypeId))
					if parameter.StorageType == DB.StorageType.Integer:
...
def setParameterAsDouble(inElement, inParameter, inValue):
	...
	if inParameter.StorageType == DB.StorageType.Double:
		try:
			...
			strToFloat = False
		if type(inValue) == float:
			convertedValue = DB.UnitUtils.ConvertToInternalUnits(inValue, forgeTypeId)
			...
			convertedValue = DB.UnitUtils.ConvertToInternalUnits(strToFloat, forgeTypeId)
			inParameter.Set(convertedValue)
