...
def convertFromInternal(inValue):
	#displayUnitType = uidoc.Document.GetUnits().GetFormatOptions(DB.ForgeTypeId).DisplayUnits
	#forgeTypeId = DB.ForgeTypeId(uidoc.Document.GetUnits().GetFormatOptions())
	forgeTypeId = DB.ForgeTypeId(DB.UnitTypeId.Millimeters.TypeId)
	#UI.TaskDialog.Show("forgeTypeId : {0}".format(forgeTypeId))
	#print("displayUnitType {}".format(displayUnitType))
	try:
		convertedValue = DB.UnitUtils.ConvertFromInternalUnits(inValue, forgeTypeId)
	except Exception as ex:
		raise ValueError("exception in convertFromInternal()", ex, sys.exc_info())
...
def convertToInternal(inValue):
	...
	try:
		...
		raise ValueError("exception in convertToInternal()", ex, sys.exc_info())

	return convertedValue
	#return DB.UnitUtils.ConvertToInternalUnits(inValue, forgeTypeId)
